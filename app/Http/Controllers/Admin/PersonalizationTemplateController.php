    ...
    private function templatePayload(PersonalizationTemplateRequest $request, ?PersonalizationTemplate $template = null): array
    {
        ...
        return [
            ...
            'preview_image_url' => $this->resolveUpload($request->file('preview_image_upload'), $request->input('preview_image_url'), $template?->preview_image_url, $request->boolean('remove_preview_image'))
                ?? $this->resolveUpload($request->file('base_template_upload'), $request->input('base_template_url'), $template?->base_template_url, false),
            ...
            'preview_data_presets' => [
                'bride_name'    => $request->input('preview_data_presets.bride_name')    ?? 'Amena',
                'groom_name'    => $request->input('preview_data_presets.groom_name')    ?? 'Hassan',
                'ceremony_date' => $request->input('preview_data_presets.ceremony_date') ?? '12 December 2026',
                'venue'         => $request->input('preview_data_presets.venue')         ?? 'Dhaka, Bangladesh',
            ],
